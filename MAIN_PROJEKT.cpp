...
public:
    ...
    // Metoda sprawdzająca, czy statek jest zatopiony
    bool isSunk() const {
        return hits == size;
    }

    // Metoda dodająca pozycję do statku
...
public:
    ...
    bool canPlaceShip(int x, int y, int size, bool horizontal) const { //Metoda sprawdzająca, czy można umieścić statek w wybranym miejscu
    ...
    bool placeShip(int x, int y, const Ship &ship, bool horizontal) { //Metoda umieszczająca statek na planszy
    ...
    bool shoot(int x, int y) {//Metoda strzelająca w wybraną pozycję
    ...
    bool allShipsSunk() const { // Metoda sprawdzająca czy wszystkie statki zostały zatopione
    ...
    void display(bool showShips) const { // Metoda wyświetlająca planszę
        cout << "  ";
...
class Player { // Klasa reprezentująca gracza
private:
    Board board;
...
public:
    void placeShips() { 
        ...
        for (const auto& ship : ships) {
            ...
            while (!placed) { // Pętla umożliwiająca umieszczenie statku na planszy
    ...
    void displayBoard(bool showShips = false) const { // Metoda wyświetlająca planszę
        board.display(showShips);
    }

    bool attack(Player &opponent, int x, int y) { // Metoda atakująca
        return opponent.board.shoot(x, y);
    }

    bool hasLost() const { // Sprawdza czy gracz przegrał
        return board.allShipsSunk();
    }
};

void clearScreen() { // Metoda do nie podglądania (:
    for (int i = 0; i < 30; ++i) {
        cout << endl;
    }
}
